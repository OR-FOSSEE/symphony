...
function [xopt,fopt,exitflag,output,lambda] =lp_linprog(varargin)
  // Solves a linear programming problem given in LP file format.
  //
  //   Calling Sequence
  //   xopt = lp_linprog(file)
  //   xopt = lp_linprog(file,param)
  //   [xopt, fopt, exitflag, output, lambda] = lp_linprog(file)
  //   [xopt, fopt, exitflag, output, lambda] = lp_linprog(file,param)
  //   
  //   Parameters
  //   file : a string describing the path to the lp file.
  ...
  //   
  //   Description
  //   OSI-CLP is used for solving the linear programming problem, OSI-CLP is a library written in C++. It reads the data for linear programming problem from the LP file and solves the problem.
  ...
  //
  // Examples
  // file1_link="https://github.com/bhanusayal/symphony/blob/master/demos/exmip1.lp";
...
//To check the number of argument given by user
   if ( rhs > 2) then
    errmsg = msprintf(gettext("%s: Unexpected number of input arguments : %d provided while should be in the set of [1 2]"), "lp_linprog",   		rhs);
    error(errmsg)
   end
   file = varargin(1);
   ...
   if (type(param) ~= 15) then
      errmsg = msprintf(gettext("%s: options should be a list "), "lp_linprog");
      error(errmsg);
   end
   if (modulo(size(param),2)) then
   errmsg = msprintf(gettext("%s: Size of parameters should be even"), "lp_linprog");
   error(errmsg);
   end
   options = list(..
      "MaxIter"     , [3000],);
   ...
   [xopt,fopt,status,iter,Zl,Dl] = rlp(file,options);
